             ...
             $(img).click(function(){
                 if(confirm("¿Quieres eliminar este informe?")){
                    ...
                    $.ajax({
                        ...
                        data: {'modo':'eliminarinforme','id_informe':id},
                        dataType: "json",
    ...
    return {
        ...
        autoJS:function(){
            ...
            function sql_mostrar(text){
                ...
                $("#sql_mostrar_normal").html(text)
                .addClass("autobold");
            }
            function sql_mostrar_subtotales(text){
                $(".autolog").removeClass("autobold");
                $("#sql_mostrar_subtotales").html(text)
                .addClass("autobold");
            }
            function sql_mostrar_total(text){
                $(".autolog").removeClass("autobold");
                $("#sql_mostrar_total").html(text)
                .addClass("autobold");
            }
...
$(function() {
    ...
    ventana.muestraFiltro =function(newid){
        ...
        for(t=0;t<filtros._filtrosListaParam.length;t++){
            ...
            if (newid==item.tipo){
                if(item["param1"]) param1 = item["param1"];
                if(item["param2"]) param2 = item["param2"];

                for(t=0;t<1000;t++){
                    if(item["param"+t])
                        params[t] = item["param"+t];
                }
                break;
            }
        }
        ...
        switch(item["tipo"]){
            case "json":
                ...
                campos = $("<select id='"+idfield1+"' "+textIfDisabled+" class='cambiosMuestraRecargar serializarpost serializaGrupos insidebox "+cssFlat+"'>");

                campos.append(  $("<option selected>").html(param1) );
                if(!esReadOnly) {
                    ...
                    $.ajax({
                        ...
                        async: true,
                        success: function(data){
                            //$("#"+idfield1).append(  $("<option>").html("test:"+item.id)  );
                            ...
                            //var ignorar2 = $(document.createElement("option")).val("__ignorar__");
                            ignorar.html("Selección multiple");

                            $field.append(ignorar);
                            //$seleccionados.append( ignorar );
                            //$no_seleccionados.append(ignorar2 );

                            for(t=0;t<data.length;t++){
                                //if(data[t]==param1){
                                if(_.include(params,data[t])){
                                    $option  = $("<option>").html(data[t]);
                                    //$option.attr("selected","selected")
                                    $option.data("modo","sel");
                            ...
                            $field.change(function(){
                                $("option:selected",$field).each(function () {
                                    ...
                                    var estadoActual = $(this).data("modo");

                                    newmodo = estadoActual=="sel"?"nosel":"sel";
                                    $(this).data("modo",newmodo);
                                    ...
                                    //console.log("moviendo valor:"+val+",a:"+newmodo);
        ...
        $("<span class='"+especialcss+"'>").attr("id","filtrosf_"+newid)
        .html(" &nbsp; "+trans[newid])
        .addClass("itemfiltro")
        .attr("rel",newid)
        ...
        console.log("agnadido:newid"+newid);
    ...
    ventana.muestraAgrupar =function(newid){
        ...
        var cerrar = $("<a href='#' class='closex'  alt='Quitar' title='Quitar'>").click(function(){
    ...
    ventana.muestraListado =    function(tipo,parametrostipo){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                document.getElementById("cajaderesultados_subtotales").innerHTML = "";
                ...
                $.ajax({
                    ...
                    success: function(data){
                        cachePaginas.barra(false);
                        document.getElementById("cajaderesultados_subtotales").innerHTML = data;
    ...
    ventana.intentaEliminarColumna = function(newid){
        ...
        if(filtros.existeAgrupar(newid)){
            ...
            AvisarUsuario("<p>No se puede eliminar columnas utilizadas en <b>agrupar</b>.");
            ...
            var notice = '<div class="notice notificador" >'
                ...
                + '<img src="images/info.png" alt="" />'
                + '<h3>Aviso</h3>'
                + '<p>No se puede eliminar columnas utilizadas en <b>agrupar</b>.</p>'
                + '</div>'
                + '<div class="notice-bottom">'
                + '</div>'
                + '</div>';
...
$(function(){
   ...
   ventana.set_cargaSucia = function(newid,modo){
       ...
       if(esta && !Sucio) { //quitar de avg
           var index = ventana._cargaEnSucio.indexOf(newid);
   ...
   /*
       ...
       if(esta && !hacerAvg) { //quitar de avg
           var index = filtros._modosSumas.indexOf(newid);
    ...
    $( ".draggable" ).draggable({
        ...
        endDrag: function(event, ui){
            ...
            if(event.pageY<limites.agrupar_y){
                filtrosagrupados_caja(event,ui);
                event.stopPropagation();
...
$(function() {
    ...
    $("#buscacampo").keyup(function(){
        ...
        comparaCon = comparaCon.toUpperCase();
        ...
        $paletas.each(function(){
            var contenido = accentsTidy($(this).val().toUpperCase());

            if(contenido.indexOf(comparaCon)<0){
                $(this).hide();
            } else {
                $(this).show();
            }
...
$(function() {
    function AjustarLayout(){
        ...
        //console.log("padarriba:"+padarriba+",alturaventana:"+alturaventana+",contamos:"+contamos);//+",newaltura:"+newaltura);
    }
...
$(function(){
   ...
   $("#toggleColpaleta").click(function(){
      ...
      estadoToggle = (estadoToggle)?false:true;


      ArreglarColumnas_limpia();
      $('#colpaleta').toggle(estadoToggle);

      $("#fila2debug").toggle(estadoToggle);
...
$(function(){
   ...
   columnas_id2tipo.agrupable = function(id){
    ...
    $(".iconito",$("#lista_columnas_icons")).each(function(){
        ...
        if(subtipe=="sum" && (tipo=="texto" || tipo=="codigo" || tipo=="cod" || tipo=="json")){
            $(this).hide();
        }
    });
