...
function creatBotonBorrarInforme(){
    ...
    $(img).click(function(){
        if(confirm("¿Quieres eliminar este informe?")){
            ...
            var id = ventana.id_informe;
            $.ajax({
                type: 'POST',
                url: 'modreporting.php',
                data: {
                    'modo':'eliminarinforme',
                    'id_informe':id
                },
                dataType: "json",
                success: function(data){
                    if(data["ok"]) {
                        $("#nombreListado").html("");
                        ventana.id_informe = 0;
                        $("#nombresugerido").val("");


                        AvisarUsuario("Informe eliminado");
                        

                        $("#listaReportesUsuario option[value="+id+"]").remove();

                        document.location = "modreporting.php?modo=inicial-limpio&r="+Math.random();
                    } else {
                        AvisarUsuario(data["error"]);
                    }
                }
            });
        }
...
function ArreglarColumnas(){
    ...
    $("#cajaderesultados tr:eq(1) td",$("#tablaresultados")).each(function(){     
            ...
            if($(this).css("display")=="none") 
                ...
                w = $(this).width();
                min1 = (w<min1)?w:min1; //registra ancho mas pequeño
            }
            tams.push(w);
            n1++;
    ...
    $("#cajaderesultados tr:eq(0) td",$("#tablaresultados")).each(function(){
            ...
            if($(this).css("display")=="none")
                ...
                w = $(this).width();
                min2 = (w<min2)?w:min1;//registra ancho mas pequeño
            }
...
function ArreglarColumnas2(pad){
    var imp,index,tams = [],tams2=[],n1=0,n2=0;
    var w,min1=99999,min2=99999,min=99999;
    ...
    if(n2>n1){
        ...
        min = min2;//usamos como minimo, el del registro 2
        //console.log("Usamos min2");
        if (n2==1){
            //console.log("Solo hay una columna!");
            return;//si hay solo una columna, no tenemos interes en usarla
        }
    } else {
        console.log("Usamos min1");
        min = min1;//usamos como minimo, el del registro 1
    }
...
$(function(){
    ...
    function intentaHastaQueExiste(){
        if(!$.datepicker || !$.datepicker.regional){
                //console.log("reintentando");

                setTimeout(function() {
                    intentaHastaQueExiste();
                },100);
                return;
       }
...
$(function() {
    ...
    ventana.desfiltrandoSubtotal = function(){

        $("#eligidoagrupar").val("");//eligidoagrupar ya no tiene nada
        $(".iconosfila").show();
        $(".cajacabeza_columna").show();
...
$(function() {
     ...
     */
        ...
        $.each(newlista,function(key,valor){
            ...
            var mensaje = "Mostrar subtotales por: "+ msgtotal;
            ...
            function esTexto(tipo){
                    ...
                    if(tipo==undefined){
                        return false; //columna virtual?
                    }
                    if(tipo=="agno") return true;
                    if(tipo=="codigo") return true;
                    if(tipo=="json") return true;
                    if(tipo=="oculto") return true;
                    if(tipo=="texto") return true;
                    if(tipo=="fecha") return true;
                    if(tipo=="porcentaje") return true;
                    if(tipo=="") return true;
            ...
            boton.click($.debounce(500,function(){
                ...
                var idtotal = localidtotal;
                console.log(idtotal);
                var campos = idtotal.split(",");
		    
                $(".cajacabeza_columna").each(function(){

                    var id = $(this).attr("id");

                    id = id.replace("thh_","");
                    var tipo = columnas_id2tipo.getTipo(id);
                ...
                $(".iconosfila").each(function(){

                    var id = $(this).attr("id");

                    id = id.replace("icons_","");
                    var tipo = columnas_id2tipo.getTipo(id);
                ...
                /*
                $(".totalcolumna").each(function(){

                    var id = $(this).attr("id");

                    id = id.replace("totalc__","");
                    var tipo = columnas_id2tipo.getTipo(id);
                    ...
                    if(esTexto(tipo)){
                        ...
                        console.log("columna:"+id+",tipo:"+tipo+",estexto");
                    }
                });*/
		    
                $.each(campos,function(key,valor){
                ...
                $(".totalcolumna").each(function(){
                    ...
                    if($(this).css("display")=="none"){
                        ...
                        console.log("eliminando oculta:"+$(this).attr("id"));
                        $(this).remove();
                    }
                });*/
 ...
 */
    filtros.modificado = function(){
        ...
        if(nombreOriginal) {
            var newnombre = nombreOriginal + " ( modificado )"
            $("#nombreListado").html(newnombre);

            $("#nombresugerido").val(newnombre);
        }
    ...
    filtros.agnadirFiltro = function(newid){
        if(jQuery.inArray(newid, filtros._filtrosLista)<0){
            ...
            filtros._filtrosLista.push(newid);
            filtros.modificado();
            ventana.sugiereReenviar(true,po_recargar);
        //filtros.necesitaRecargar(true);//necesita recargar toda la pagina
        }
...
$(function() {
    ...
    /* Limita textos a una longitud data, corta si se salen */
    function cortaLargos(texto,longitud){
        texto = texto + "";

        if(!longitud) longitud = 20;
        if(texto.length<longitud) return texto;

        var newtexto = texto.substring(0, longitud) + "...";
        return newtexto;
    }

    /*
     * Construye un combo con opciones descargadas adecuadas para este combo
     */
    ventana.build_json_combo = function(esReadOnly,newid,param1,params,item){
        ...
        var nulo =  $("<option selected='selected' value=''>Cargando..</option>");
        nulo.val(param1);
        campos.append( nulo );
        ...
        if(!esReadOnly) {
            ...
            var procesaRetorno = function(data){
                ...
                if(data)
                    for(t=0;t<data.length;t++){
                        ...
                        if(!_.include(params,valoption)){
                            ...
                            $option.attr("data-orden-b",t+1);//punto de insercion
                ...
                if(data && autodescribeIgnorar){
                    if(1) ignorar.html( cortaLargos(autodescribeIgnorar) );
                }
                autodescribeIgnorar = "";


                function calculaIndiceInsercion(idfield1,texto,clase){
                    var salida = [];
                    var names = "#"+idfield1+" ."+clase+" option";

                    $(names).each(function(){
                         salida.push( $(this).text() );
                    });
                    ...
                    var salida2 = [];
                    salida2 = salida.slice();

                    salida2.push(texto);
                    salida2 = salida2.sort();

                    var t=0;
                    var index=-1;
                    for(t=0;t< salida.length;t++){
                        if(salida[t]!=salida2[t]){
                            index=t;
                            break;
                         }
                    }
                    ...
                    setTimeout(function(){
                        ...
                        $(".mmarcado",$field).each(function () {
                            ...
                            if(newmodo=="sel"){
                                ...
                                var indice = calculaIndiceInsercion(idfield1,val,"disponibles");

                                console.log("val:"+val+",indice:"+indice);

                                if(indice==-1){
                                        //$selcaja
                                        $(this).insertAfter( $no_seleccionados.find("option:first") );
                                } else if (indice==-2){
                                        $no_seleccionados.append(this);
                                } else {

                                        console.log("moviendo a no seleccionados");
                                        indice --;

                                        var element = $no_seleccionados.find("option").eq(indice);
                        ...
                        if ( autodescribeIgnorar && autodescribeIgnorar!="")
                            $field.find("[value=__ignorar__]").html(cortaLargos(autodescribeIgnorar));
                        else
                        ...
                        if($.browser.msie){
                            ...
                            if(agnadirNoSeleccionado!=codigoSeguro){
                                ...
                                if(!encontrado){
                                    $selcaja = $("optgroup.disponibles",$field);
    ...
    ventana.muestraFiltro =function(newid){
        ...
        var campos,idfield1,idfield2, especialcss;
        ...
        switch(item.tipo){
            ...
            case "normal":
                ...
                campos =  $("<input type='text' value='"+param1+"'  "+textIfDisabled+" size='8' id='param_filtro_"+newid+"' class='cambiosMuestraRecargar insidebox serializarpost "+cssFlat+"'>");
        ...
        var sentido = "igual";
        var shortid = newid.replace(columnas_id2tipo._tabla+"_","");

        if ( filtros._filtrosModoFiltro[shortid]){
            sentido = filtros._filtrosModoFiltro[shortid];
            //console.log("sentido detectado:"+sentido);
        }
        //console.log("sentido("+shortid+"):"+sentido);



        var igual = $("<a href='#' title='igual que'><img border='0' style='border:0px' class='modofiltro' data-newid='"+newid+"' src='icons/igual.gif' align='absmiddle' data-sentido='"+sentido+"' id='modo_filtro_"+newid+"'></a>");

        var fichatipo="moneda", ficha;
        try {
            ficha = _.find(columnas,function(dato){if(dato.id==newid) return dato; else return false});
            fichatipo = ficha["tipo"];
        } catch(e){

        }

        function set_tipo(enlace,sentidoActual){
            console.log("este, sentido:"+sentidoActual);

            var img = $("img",enlace);

            //console.log(img.attr("src"));

            if(sentidoActual=="distinto"){

                //console.log("distinto!!!!!!!!!!")
                $(img).attr('src',"icons/notequal.gif")
                .attr("border","0")
                .attr("data-sentido","distinto");

                $(enlace).attr("title","distinto de")

            } else
            if(sentidoActual=="mayorque")
            {
                $(img).attr('src',"icons/mayorque.gif")
                .attr("border","0")
                .attr("data-sentido","mayorque");
                $(enlace).attr("title","mayor que")
            }else
            if(sentidoActual=="menorque") {
                $(img).attr('src',"icons/menorque.gif")
                .attr("border","0")
                .attr("data-sentido","menorque");

                $(enlace).attr("title","menor que")
            }else{
                $(img).attr('src',"icons/igual.gif")
                .attr("border","0")
                .attr("data-sentido","igual");

                $(enlace).attr("title","igual que")
            }
        }

        
        set_tipo(igual,sentido);


        igual.click(function(){

            var este = $("img",this);
            var sentidoActual = $(este).attr("data-sentido");

            console.log("igual.click:"+sentidoActual);
            
            if(sentidoActual=="igual"){
                set_tipo(this,"distinto");
            } else if(sentidoActual=="distinto"  && fichatipo=="moneda") {
                set_tipo(this,"mayorque");
            } else if(sentidoActual=="mayorque"  && fichatipo=="moneda") {
                set_tipo(this,"menorque");
            } else {
                set_tipo(this,"igual");
            }
        })
        ...
        $("#filtrosfiltros").append(
            $("<div class='"+especialcss+"'></div>").attr("id","filtrosf_"+newid)
            .html(" &nbsp; "+trans[newid]+" &nbsp;")
            .append(igual)
            .addClass("itemfiltro")
            .attr("rel",newid)
        ...
        ventana.limites= {
            agrupar_y:$("#filtrosfiltros_caja").offset().top,
            filtros_y:$("#botonRecargarVoluntario2").offset().top
        };
    ...
    ventana.muestraAgrupar = function(newid){
        ...
        var cerrar = $("<a href='#' class='closex'  alt='Quitar' title='Quitar'>X</a>").click(function(){
 ...
 */
    ventana.muestraListado = function(tipo,parametrostipo){
        ...
        $.ajax({
            ...
            async: true,
            success: function(data){
                cachePaginas.add(paginaPeticion,data);//cacheamos esta pagina
                cachePaginas.barra(false);
 ...
 */
    ...
    ventana.serializa_modosfiltro = function(){

        var data = "{";
        var coma = "";

        $("img.modofiltro").each(function(){
            var cual = $(this).attr("data-newid");
            var modo = $(this).attr("data-sentido");
        ...
        //var data_json = json_serialize(data);

        $("#imodosfiltros").val(data);
...
/*
 * Hace un mantenimiento del estado de los json, para evitar lanzar una consulta antes
 * de que todos los json tengan un valor valido (Solo tienen el valor valido tras cargarse)
 */
$(function(){
...
$(function(){
    ...
    ventana.evento_agnadirfiltrosagrupados_caja = function(newid){
        ...
        if( jQuery.inArray(newid, filtros.getAgrupar())<0){ //es nuevo filtro (sino, lo ignora)
            ...
            var col = -1;
            try {
                col = _.compact(filtros.getColumnas()).length;
            } catch(e){};

            //console.log("cols"+cols.length);

            filtros.agnadirColumna(newid,col);//añadimos este filtro a las columnas que se visualizaran
            ventana.sugiereReenviar(true,po_recargar);


            console.log("agnadiendo newid:"+newid+",col:"+col);
...
$(function(){
    ...
    ventana.filtrosagrupados_caja =
    function(evento,draggable ) {
    ...
    ventana.filtrosubtotales_caja = function ( evento,draggable ) {
        var newid = $(draggable).attr("rel")+"";
    ...
    ventana.filtrosfiltros_caja = function ( evento, draggable ) {
        var newid = $(draggable).attr("rel")+"";
        ...
        if(jQuery.inArray( newid,filtros.getFiltros() )<0){
            ...
            if(permisos.filtro_agnade_columna)
    ...
    ventana.calculaColumna =  function (clickx){
     ...
     */
    ventana.calculaMediaColumna =  function (clickx){
        var columnas = $("#lista_columnas th");
        var len = columnas.length;
        var tamahora=ventana.estadoToggle?240:0;//columna izquierda
        var columnaMoverTo = -1;
    ...
    ventana.movercolumnas = function ( event, draggable ) {
        ...
        if($(draggable).hasClass("cabeza_columna")){
            ...
            var columnaMoverTo = ventana.calculaColumna(clickx);
    ...
    ventana.drag_termina = function (event, ui){
        var newid;
        ...
        var itemdepaleta = (pasar &&  $(pasar).hasClass("itemdepaleta"));
        ...
        if(event.pageY<ventana.limites.agrupar_y){
            ventana.filtrosagrupados_caja(event,pasar);
            event.stopPropagation();
            //alert(2);
            return;
        } /* else if (event.pageY<ventana.limites.subtotal_y)  {
            filtrosubtotales_caja(event,ui);
            event.stopPropagation();

            console.log("c:1044");
            return;
        } */else if (event.pageY<ventana.limites.filtros_y)  {
            ventana.filtrosfiltros_caja(event,pasar);
            event.stopPropagation();
            ...
            if($.browser.msie)
                ventana.movercolumnas(event,event.target);
            else
                ventana.movercolumnas(event,pasar);
            ...
            newid =  $(pasar).attr("rel");

            if(!permisos.agnadir_columnas_engrid){
                console.log("permisos.agnadir_columnas_engrid(bloquea): No se permite meter columnas de esta manera para este usuario");
                event.stopPropagation();
                return;
            }


            var columnaMoverTo = ventana.calculaMediaColumna(event.pageX);
...
function hacerArrastrable(){
    ...
    $( ".draggable" ).draggable({
        ...
        endDrag: function (event, ui) {ventana.drag_termina(event,ui);} ,
        stop: function (event, ui) {ventana.drag_termina(event,ui);}
    });
...
$(function() {
    ...
    function agnadirItemPaleta(item){
        ...
        $linea.addClass("itemdepaleta ocupado linear draggable  "+extraclass);
...
$(function() {
    ...
    $("#salvareporte").click(function(){
        ...
        $("#listaReportesUsuario option.personal").each(function(){
            var text = $(this).text();

            if( text == nombre_para_informe ) existe = true;
            //console.log("nombre_para_informe:"+nombre_para_informe+",!="+text);
        });

        if(existe){
            alert("No se puede guardar por este nombre, porque ya existe");
            return;
        }
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data["ok"]){
                    ...
                    document.location = "modreporting.php?modo=loadreport&id="+data.id+"&r="+Math.random();
                }
            }
        });
        return false;
    ...
    $("#guardarcambios").click(function(){
        ...
        var po_guardandoinforme  = "Guardar cambios: ¿Quieres modificar el informe actual?";


        var id_reporte = ventana.id_informe;

        if(!id_reporte){
            AvisarUsuario("No se puede modificar");
            return;
        }

        if(!confirm(po_guardandoinforme)){
            return;
        }
        
        $("#idmodosharereport").val(id_reporte);//que reporte estamos actualizando
        $("#modoform").val("actualizareport");
        ventana.serializarParametros();
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data["ok"]){
                    ...
                    AvisarUsuario("Cambios guardados");
                } else {
    ...
    $("#listaReportesUsuario").change(function(){
        var id = $(this).val();
        if(!id) return;
        document.location = "modreporting.php?modo=loadreport&id="+id;
    });
...
$(function() {
    function AjustarLayout(){
        ...
        var alturaventana = $(window).height() - padabajo;
...
$(function(){
    ...
    $("#compartirlistado").change(function(){
        ...
        var txtquien =  $("#compartirlistado option:selected").text();
            ...
            success: function(data){
                ...
                if(data["ok"]){
                    ...
                    if(quien=="otrousuario")
                        AvisarUsuario("El listado se ha compartido correctamente con "+idusuario);
                    else
                        alert("El listado se ha compartido correctamente con "+ txtquien);
...
/*
* Gestiona visibilidad de iconitos
*
*/
$(function(){
...
/*
* FUNCIONES DE AYUDA AL DEBUGGING / PARCHEADO DE BUGS DE IE
*/
...
$(function(){
    ...
    $("#boton_cancelador").click(function(){
        $("#cancelador").submit();
    });
...
/*
* Oculta features que no se quieren mostrar.
*
*/
$(function(){

    if(!permisos.mostrar_subtotales){
        $("#cajaMenuSubtotales").hide();
    }

   
    if(!permisos.botones_crud_informe){
        $("#accion_reinicio").hide();
        $("#guardarcambios").hide();
        $("#salvareporte").hide();
    }


    if(!permisos.debug_sql){
        $("#boton_MostrarSQL").hide();
    }
