...
class xlsStream {
                ...
                if (is_array($_data) && !empty($_data)) {
                    if ($row == 0) {
                        ...
                        foreach (array_keys($_data) as $col => $val) {
                            ...
                            if (function_exists("code2nombre")) {
                                $val = code2nombre($val);

                                $val = iconv("UTF8", "iso-8859-1//TRANSLIT", $val);
                            }
    ...
    function _xlsWriteCell($row, $col, $val) {
        ...
        $dato = str_replace(",", ".", $val); //formato americano

        $es_numerico = strval(floatval($dato)) == "$dato";

        if (is_float($val) || is_int($val) || is_numeric($val) || $es_numerico) {
            //$val = str_replace(".",",",$val);//formato americano
            // doubles, floats, integers
            //error_log("escribe num:($val)");
