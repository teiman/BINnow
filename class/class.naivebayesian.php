...
class NaiveBayesian
{
    ...
    */
    function categorize($document,$box_id=false){    

        if($box_id){
            $this->selectBox($box_id);
        }
    ...
    */
    function train($doc_id, $category_id, $content,$box_id=false)
    {
        if($box_id)
            $this->nbs->selectBox($box_id);
    ...
    */
    ...
    function eliminaCategoria($cat_id){
        $this->nbs->eliminaCategoria($cat_id);
        $this->updateProbabilities();
    }

    //$nb->train($docid, $cat, $doc)
    function entrenar($nombre,$cat_id,$texto,$box_id=false){
        //TODO: usar nbs
        $sql = "SELECT max(id) as max FROM  nb_references";
        $row = queryrow($sql);

        //TODO: esto es seguro?
        $id = intval($row["max"] + 1);
        return $this->train($id,$cat_id,$texto,$box_id);
    }

    function creaCategoria($nombre,$box_id=false){
        if($box_id)
            $this->nbs->selectBox($box_id);
        $this->nbs->creaCategoria($nombre);
    }
    ...
    function preparaTexto($text){
        ...
        foreach($lineas as $linea){
                ...
                for($t=0;$t<$c;$t++){
                        ...
                        if(strlen($word)<2 or !ctype_graph($word)){
                        //if(strlen($word)<2 or !ctype_alpha($word)){
                                //delete $words[$t];
                        }	else {
                                $salida = $salida . " " . $word;
                        }
